...
def cargado():
    if request.method == 'POST':
        ...
        consultas = Task.query.filter_by(cedula=cedula)
        # fechas = []
        # motivos = []
        # for consulta in consultas:
        #     motivos.append(consulta[0].motivo_de_consulta)
        #     fechas.append(consulta[0].fecha)

        return render_template('usuario.html', consultas=consultas)#, motivos=motivos, fechas=fechas)
...
@app.route('/detalle-consulta/<consulta_id>/')
def detalle_consulta(consulta_id):
    consulta = Task.query.filter_by(id=consulta_id).first()
    return render_template('consulta_paciente.html', consulta=consulta)
